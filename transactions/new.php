...
<body class="bg-slate-900">
    <main class="max-w-xl m-auto p-4">
        ...
        <form action="new.php" method="post" class="m-auto w-full md:w-50 flex flex-col">
            <label for="sender" class="block text-slate-200">Choose the Sender</label>
            <select required name="sender" id="sender" class="input-control">
                <option value="" disabled selected>--Please choose an option--</option>
                <?php foreach ($list as $i) : ?>
                    <option value="<?= $i[0] ?>"><?= $i[0] . ". " . $i[1] ?></option>
                <?php endforeach ?>
            </select>
            <label for="receiver" class="block text-slate-200">Choose the Receiver</label>
            <select required name="receiver" id="receiver" class="input-control <?php if ($errflag == 1) echo 'ring-4 ring-red-500' ?>">
                <option value="" disabled selected>--Please choose an option--</option>
                <?php foreach ($list as $i) : ?>
                    <option value="<?= $i[0] ?>"><?= $i[0] . ". " . $i[1] ?></option>
                <?php endforeach ?>
            ...
            <?php if ($errflag === 1) : ?>
                <p class="text-slate-400">Self-transfers are not allowed</p>
            <?php endif; ?>
            <label for="amount" class="block text-slate-200">Amount</label>
            <input required type="number" name="amount" id="" max="10000.00" class="input-control <?php if ($errflag == 2) echo 'ring-4 ring-red-500' ?>">
            <?php if ($errflag === 2) : ?>
                <p class="text-slate-400">Not enough balance</p>
            <?php endif; ?>
